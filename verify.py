...
def discord_callback():
    ...
    try:
        ...
        # Benutzer und Verknüpfungen in der Session speichern
        ...
        # Zeige Informationen über verknüpfte Accounts an
        if connections:
...
def verify_video():
    ...
    # Überprüfe, ob der Benutzer bereits einen Verifizierungsversuch in den letzten 24 Stunden durchgeführt hat
    user_id = session['user']['id']
    
    # Lade bestehende Cooldowns aus der Datei
    cooldowns = {}
    ...
    # Erstelle einen eindeutigen Schlüssel für diesen Benutzer und diese Plattform
    cooldown_key = f"{user_id}_{platform}"
    current_time = datetime.datetime.now()
    
    # Überprüfe, ob ein Cooldown aktiv ist
    if cooldown_key in cooldowns:
        ...
        if time_diff.total_seconds() < 24 * 60 * 60:
            ...
            # Speichere Cooldown-Info auch in der Session für die Anzeige
    ...
    # Aktualisiere den Zeitpunkt des letzten Verifizierungsversuchs
    cooldowns[cooldown_key] = current_time.isoformat()
    
    # Speichere Cooldown-Info auch in der Session für die Anzeige
    session[f"last_attempt_{platform}"] = current_time.isoformat()
    
    # Speichere die aktualisierten Cooldowns in der Datei
    try:
        with open(COOLDOWN_FILE, 'w') as f:
    ...
    # Extrahiere Informationen aus dem Video-Link
    ...
    # Überprüfe, ob der Benutzer einen verknüpften Account für diese Plattform hat
    user_data = session['user']
    connections = session['connections']
    
    # Extrahiere den Plattform-Benutzernamen aus dem Video-Link
    if platform == 'tiktok':
        ...
        # Suche nach TikTok in den verknüpften Accounts
        ...
        # Speichere Verifizierungsdaten in Excel
        send_webhook_notification(user_data, platform, platform_username, video_link)
        
    elif platform == 'youtube':
        # Für YouTube müssen wir den Kanalnamen aus der API abrufen
        ...
        # Suche nach YouTube in den verknüpften Accounts
        ...
        # Verwende die YouTube API, um Informationen über das Video zu erhalten
        try:
            if YOUTUBE_API_KEY:
                ...
                if video_data.get('items'):
                    ...
                    # Überprüfe, ob der Kanalname mit dem verknüpften YouTube-Account übereinstimmt
...
def extract_video_info(platform, video_link):
    ...
    if platform == 'tiktok':
        ...
        # TikTok-Kurzlink-Format: https://vm.tiktok.com/XXXXXXXX/
        if 'vm.tiktok.com' in video_link:
            try:
                ...
                # Extrahiere Informationen aus dem endgültigen Link
        ...
        # YouTube-Kurzlink-Format: https://youtu.be/xxxxxxxxxxx
...
def send_webhook_notification(user_data, platform, platform_username, video_link):
    ...
    if os.path.exists(EXCEL_FILE):
        try:
            ...
            # Erstelle eine neue Datei, wenn die bestehende nicht geladen werden kann
            wb = Workbook()
            ws = wb.active
...
def auth_logout():
    ...
    # Entferne auch alle Cooldown-Informationen aus der Session
